...
export function Footer() {
  return (
    ...
    >
      {/* Top separator — feather gradient, not hard border */}
      <div className="divider" />

      {/* Ambient void glow — dark mode only, felt not seen */}
      <div
        className="dark:block hidden pointer-events-none absolute inset-0"
        style={{
          background: 'radial-gradient(ellipse 60% 40% at 20% 0%, rgba(224,0,30,0.025) 0%, transparent 70%)',
        }}
      />

      <div className="container-wide" style={{ paddingTop: 'clamp(3rem, 5vw, 5rem)', paddingBottom: 'clamp(2rem, 3vw, 3rem)', position: 'relative' }}>

        {/* ── Main grid ─────────────────────────────────────────────── */}
        <div className="grid grid-cols-2 md:grid-cols-6 gap-10 lg:gap-16">

          {/* ── Brand column ────────────────────────────────────────── */}
          <div className="col-span-2 flex flex-col" style={{ gap: 'clamp(1.25rem, 2vw, 1.75rem)' }}>
            ...
            {/* Social icons — void-surface tokens, not generic gray boxes */}
            ...
            {/* Location — feather pill, accent pin */}
            <div
              className="inline-flex items-center gap-1.5 self-start"
              style={{
                borderRadius: 'var(--radius-pill)',
                background:   'hsl(var(--surface-elevated))',
                border:       '1px solid hsl(var(--border-feather))',
          ...
          {Object.entries(footerLinks).map(([title, links]) => (
            <div key={title}>
              <h4 className="eyebrow" style={{ marginBottom: '1.25rem' }}>
                {title}
              </h4>
        ...
        {/* ── Bottom bar ──────────────────────────────────────────────── */}
        <div
          style={{ marginTop: 'clamp(3rem, 5vw, 4.5rem)', paddingTop: '1.75rem', position: 'relative' }}
        >
          {/* Feather divider */}
          ...
          <div className="flex flex-col sm:flex-row items-center justify-between gap-3">
            ...
            {/* Editorial terminal mark — centre */}
            <span
